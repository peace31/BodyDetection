...
def get_batch_spec(cfg):
    ...
    batch_spec = {
        pose_dataset.Batch.inputs: [batch_size, None, None, 3],
        pose_dataset.Batch.part_score_targets: [batch_size, None, None, num_joints],
        pose_dataset.Batch.part_score_weights: [batch_size, None, None, num_joints]
    }
    if cfg.location_refinement:
        batch_spec[pose_dataset.Batch.locref_targets] = [batch_size, None, None, num_joints * 2]
        batch_spec[pose_dataset.Batch.locref_mask] = [batch_size, None, None, num_joints * 2]
    if cfg.pairwise_predict:
        batch_spec[pose_dataset.Batch.pairwise_targets] = [batch_size, None, None, num_joints * (num_joints - 1) * 2]
        batch_spec[pose_dataset.Batch.pairwise_mask] = [batch_size, None, None, num_joints * (num_joints - 1) * 2]
    return batch_spec
...
class PoseNet:
    ...
    def part_detection_loss(self, heads, batch, locref, pairwise, intermediate):
        ...
        part_score_weights = batch[pose_dataset.Batch.part_score_weights] if weigh_part_predictions else 1.0

        def add_part_loss(pred_layer):
            return tf.losses.sigmoid_cross_entropy(batch[pose_dataset.Batch.part_score_targets],
                                                   heads[pred_layer],
                                                   part_score_weights)
        ...
        if locref:
            ...
            locref_targets = batch[pose_dataset.Batch.locref_targets]
            locref_weights = batch[pose_dataset.Batch.locref_mask]
        ...
        if pairwise:
            ...
            pairwise_targets = batch[pose_dataset.Batch.pairwise_targets]
            pairwise_weights = batch[pose_dataset.Batch.pairwise_mask]
    ...
    def train(self, batch):
        ...
        heads = self.get_net(batch[pose_dataset.Batch.inputs])
